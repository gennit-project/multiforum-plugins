// ChatGPT Bot Profiles plugin
// Responds to /bot mentions in comments with ChatGPT-generated replies.
...
export default class ChatgptBotProfiles {
  ...
  private getEffectiveConfig() {
    ...
    const defaultProfileId = isNonEmptyString(channel.defaultProfileId)
      ...
      : isNonEmptyString(rawSettings.defaultProfileId)
        ...
        : server.defaultProfileId || (profiles[0]?.id || "general");
    ...
    for (const profile of uniqueProfiles.values()) {
      try {
        ...
        this.context.logPromptDebug?.({
          label: `chatgpt-bot-profiles:${profile.id}`,
          prompt: userPrompt,
